...
func TestLooksLikeSQLBoundaryChecks(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "valid select", in: "SELECT * FROM users", want: true},
		{name: "valid insert", in: "INSERT INTO users(id) VALUES (1)", want: true},
		{name: "keyword followed by identifier char", in: "SELECTX * FROM users", want: false},
		{name: "keyword with underscore suffix", in: "UPDATE_users", want: false},
		{name: "comment start", in: "-- hello", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := looksLikeSQL(tt.in)
			if got != tt.want {
				t.Fatalf("looksLikeSQL(%q) = %v, want %v", tt.in, got, tt.want)
			}
		})
	}
}
