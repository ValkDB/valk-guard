...
func TestJSONReporter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if envelope.Version == "" {
				t.Error("expected non-empty version in envelope")
			}

			if len(envelope.Findings) != tt.wantCount {
				t.Errorf("expected %d findings, got %d", tt.wantCount, len(envelope.Findings))
			}

			if envelope.Summary.Total != tt.wantCount {
				t.Errorf("expected summary.total %d, got %d", tt.wantCount, envelope.Summary.Total)
			}

			if tt.wantFirstRule != "" && len(envelope.Findings) > 0 {
				if envelope.Findings[0].RuleID != tt.wantFirstRule {
					t.Errorf("expected first rule %s, got %s", tt.wantFirstRule, envelope.Findings[0].RuleID)
				}
			}
		})
	}
