...
type Registry struct {
	...
	allIDs map[string]struct{} // cross-type ID uniqueness check
}
...
func (r *Registry) Register(rule Rule) error {
	id := rule.ID()
	if _, exists := r.allIDs[id]; exists {
...
func DefaultRegistry() *Registry {
	...
	mustRegisterSchema(reg, &OrphanMigrationTableRule{})
	mustRegisterSchema(reg, &DuplicateModelColumnMappingRule{})
	mustRegisterSchema(reg, &GoInferredTableNameRiskRule{})

	mustRegisterQuerySchema(reg, &UnknownProjectionColumnRule{})
	mustRegisterQuerySchema(reg, &UnknownFilterColumnRule{})
	mustRegisterQuerySchema(reg, &UnknownTableReferenceRule{})
	mustRegisterQuerySchema(reg, &AmbiguousUnqualifiedColumnRule{})
