...
def _extract_columns(class_node):
    """Extract Column() and mapped_column() definitions from a class body."""
    columns = []
    for stmt in class_node.body:
        # Handle: attr = Column(...) / attr = mapped_column(...)
        ...
        # Handle: attr: Mapped[Type] = mapped_column(...)
            ...
            if not _is_column_call(stmt.value):
                continue
            col_name, col_type = _parse_column_args(stmt.value, field_name)
            # If no type from args, try to extract from Mapped[Type] annotation
            if not col_type and stmt.annotation:
                col_type = _type_from_mapped_annotation(stmt.annotation)
            columns.append({
                "name": col_name,
                "field": field_name,
                "type": col_type,
...
def _is_column_call(call):
    """Check if a call node is Column(...) or mapped_column(...)."""
    func = call.func
    if isinstance(func, ast.Name) and func.id in ("Column", "mapped_column"):
        return True
    if isinstance(func, ast.Attribute) and func.attr in ("Column", "mapped_column"):
        return True
    return False


def _type_from_mapped_annotation(annotation):
    """Extract type from Mapped[Type] annotation."""
    if not isinstance(annotation, ast.Subscript):
        return ""
    # Check it's Mapped[...]
    func = annotation.value
    if isinstance(func, ast.Name) and func.id == "Mapped":
        return _type_from_node(annotation.slice)
    if isinstance(func, ast.Attribute) and func.attr == "Mapped":
        return _type_from_node(annotation.slice)
    return ""
