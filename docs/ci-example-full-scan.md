## CI Example: Full-Repo PR Scan

Copy-paste this when you want the shortest possible GitHub Actions setup.

Use it when:

- the repo is small
- `main` is already clean
- you are okay scanning the whole repo on each PR

```yaml
name: valk-guard-pr

on:
  pull_request:
    branches: [main]

permissions:
  contents: read
  pull-requests: write

env:
  VALK_GUARD_INSTALL_REF: vX.Y.Z

jobs:
  scan:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4

      - uses: actions/setup-go@v5
        with:
          go-version: "1.25.6"

      - name: Install valk-guard
        run: go install github.com/valkdb/valk-guard/cmd/valk-guard@${VALK_GUARD_INSTALL_REF}

      - uses: reviewdog/action-setup@v1

      - name: Run valk-guard
        run: |
          set +e
          valk-guard scan . --config .valk-guard.yaml --format rdjsonl > valk-guard.rdjsonl
          code=$?
          set -e

          if [ "$code" -gt 1 ]; then
            exit "$code"
          fi

      - name: Post PR review comments
        env:
          REVIEWDOG_GITHUB_API_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: |
          reviewdog \
            -f=rdjsonl \
            -name="valk-guard" \
            -reporter=github-pr-review \
            -filter-mode=added \
            -fail-level=none \
            < valk-guard.rdjsonl
```

Install choices:

- local dev: `go install github.com/valkdb/valk-guard/cmd/valk-guard@latest`
- CI release pin: `go install github.com/valkdb/valk-guard/cmd/valk-guard@vX.Y.Z`
- CI commit pin: `go install github.com/valkdb/valk-guard/cmd/valk-guard@<commit-sha>`
